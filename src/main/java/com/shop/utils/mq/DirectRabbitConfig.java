...
public class DirectRabbitConfig {
    ...
    //队列 起名：operLogQueue
    @Bean
    public Queue operLogQueue() {return new Queue("operLogQueue",true);  //true 是否持久
         }

    //Direct交换机 起名：OperLogExchange
    @Bean
    DirectExchange OperLogExchange() {
        return new DirectExchange("OperLogExchange");
    }

    //绑定  将队列和交换机绑定, 并设置用于匹配键：OperLogRouting
    @Bean
